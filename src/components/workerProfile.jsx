...
function Review({onDelete, id, image, name, comment, stars, date }) {
  function handelDeleteBtn() {
    axios
      .delete(`http://localhost/brikolZone/deleteReview.php?reviewID=${id}`, {
...
const UpdateProfileForm = ({ OldData, onSubmit, onClose }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost/brikolZone/getJobCategoriesList.php`, {
        withCredentials: true,
      })
      .then((response) => {
    ...
    if (newFormData.image) {
     ...
     try {
       const response = await axios.post(
         `http://localhost/brikolZone/updateWorkerInfo.php?workerID=${workerID}`,
         formData,
         {
           headers: {
 ...
 const getWorkerInfo = async () => {
   try {
     const response = await axios.get(
       `http://localhost/brikolZone/getWorkerInfo.php?workerID=${workerID}`,
       { withCredentials: true }
     );
