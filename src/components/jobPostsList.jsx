...
export default function AdminJobPosts() {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await axios.post(
          "http://localhost/brikolZone/getJobPostsList.php",
          {
            searchTerm,
            filter,
          },
  ...
  const handleDelete = async (id) => {
    if (
      ...
      try {
        await axios.delete(
          `http://localhost/brikolZone/deleteJobPost.php?id=${id}`,
          {
            withCredentials: true,
          }
      ...
      <div className="space-y-6">
        {jobPosts.length === 0 ? (
          ...
          jobPosts.map((post) => (
            <div
              key={post.jobPostID}
              className={`border rounded-xl p-5 shadow-md transition duration-300  `}
            >
                ...
                <p className="text-sm mt-1 text-gray-500">
                  ...
                  <span
                    className={`font-semibold ${
                      post.client.isDeleted ? "text-red-600" : "text-green-600"
                    }`}
                  >
